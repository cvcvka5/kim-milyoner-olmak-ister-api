...
const App = () => {
  ...
  const getNextQuestion = async () => {
    try {
      ...
      if (hasAudioQuestion) {
        question = await getRandomQuestion({nth: nextNumber, audio: false});
      }
      // If we're at question 5 and haven't had an audio question yet, force one
      else if (nextNumber === 5) {
        question = await getRandomQuestion({nth: nextNumber, audio: true});
        setHasAudioQuestion(true);
      }
      // Otherwise, allow random chance for audio question (but not guaranteed)
      else {
        question = await getRandomQuestion({nth: nextNumber});
  ...
  const handleAnswerSelect = async (_answer: string, answerKey: string) => {
    ...
    // Calculate suspense delay based on question number
    const getSuspenseDelay = (questionNumber: number): number => {
      if (questionNumber <= 8) {
        return Math.min(questionNumber, 6); // 1, 2, 3, 4, 5, 6, 6, 6 seconds
      }
      return 6; // Stay at 6 seconds for questions 9+
    };
    ...
    }, 275); // Flash every 275ms
    
    // Add suspense delay before showing result
    await new Promise(resolve => setTimeout(resolve, delay * 1000));
    
    // Stop tension sound immediately when answer is revealed
    if (audioContext) {
      audioContext.close();
    }
    
    // Stop flashing and show result
    ...
    // Check if this is a win (12th question correct)
    if (currentQuestionNumber === 12 && isCorrect) {
      setHasWon(true);
        ...
        <div className="answers-container">
          {currentQuestion && Object.entries(currentQuestion.choices).map(([key, answer]) => (
            <button
              ...
              disabled={showResult || isInSuspense}
            >
            ...
            {hasWon ? (
              <div className="celebration-container">
                ...
                <div className="win-message">
                  ...
                  <p className="win-subtitle">1.000.000 ₺</p>
                </div>
                <button className="btn btn-primary btn-lg" onClick={startGame}>
                  Baştan Başla
                </button>
